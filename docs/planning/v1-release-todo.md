...
- [ ] **Global Quick Summon** — OS-level shortcut (e.g., `Alt+Space`) to open Atlas from anywhere.
- [ ] **Live File Watching** — Implement Rust `notify` crate to watch the workspace and auto-index file changes in real-time.
- [ ] **Native Git Awareness** — Understand branches, commit history, and active uncommitted diffs to enrich contextual RAG.
- [ ] **Editor Handoff** — "Open in VS Code" buttons on code snippets and files referenced by the LLM.
- [x] **UI Polish & Fixes**:
...
## 🤖 Phase 6: Agentic Capabilities & Actions (v0.7.0)
- [ ] **Tool Calling Engine** — Equip Atlas with the ability to safely execute bounded shell commands (read logs, run tests).
- [ ] **Workspace Code Application** — Allow Atlas to propose code diffs and apply them directly to local files with user confirmation.
- [ ] **Multi-step Reasoning UI** — Support and visualize "thinking" chains (Chain of Thought), similar to DeepSeek-R1 or OpenAI o1.
- [ ] **Semantic Code Parsing** — Use `tree-sitter` in Rust to index structural code definitions (functions, classes) vs. raw text chunks.

## 🕸️ Phase 7: Knowledge Graph & Context Mastery (v0.9.0)
...
## 🚀 Phase 8: v1.0.0 Launch Readiness (v1.0.0)
